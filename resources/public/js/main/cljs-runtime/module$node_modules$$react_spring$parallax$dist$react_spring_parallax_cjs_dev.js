shadow$provide.module$node_modules$$react_spring$parallax$dist$react_spring_parallax_cjs_dev=function(global,require,module,exports){function _extends(){_extends=Object.assign||function(target){for(var i=1;i<arguments.length;i++){var source=arguments[i],key;for(key in source)Object.prototype.hasOwnProperty.call(source,key)&&(target[key]=source[key])}return target};return _extends.apply(this,arguments)}function _objectWithoutPropertiesLoose(source,excluded){if(null==source)return{};var target={},sourceKeys=
Object.keys(source),i;for(i=0;i<sourceKeys.length;i++){var key=sourceKeys[i];0<=excluded.indexOf(key)||(target[key]=source[key])}return target}function getScrollType(horizontal){return horizontal?"scrollLeft":"scrollTop"}function mapChildrenRecursive(children,callback){return React__namespace.Children.map(children,child=>{var JSCompiler_inline_result=child.type?child.type===React__namespace.Fragment:child===React__namespace.Fragment;return JSCompiler_inline_result?mapChildrenRecursive(child.props.children,
...
({config,horizontal,busy:!1,space:0,current:0,offset:0,controller:new web.Controller({scroll:0}),layers:new Set,update:()=>update$jscomp$0(),scrollTo:offset=>scrollTo(offset),stop:()=>state.controller.stop()}),[]);React.useEffect(()=>{state.config=config},[config]);React__namespace.useImperativeHandle(ref,()=>state);const containerRef=React.useRef(),contentRef=React.useRef(),update$jscomp$0=()=>{var container=containerRef.current;if(container){state.space=container[horizontal?"clientWidth":"clientHeight"];
var scrollType=getScrollType(horizontal);enabled?state.current=container[scrollType]:container[scrollType]=state.current=state.offset*state.space;(container=contentRef.current)&&(container.style[horizontal?"width":"height"]=`${state.space*pages}px`);state.layers.forEach(layer=>{layer.setHeight(state.space,!0);layer.setPosition(state.space,state.current,!0)})}},scrollTo=offset=>{const container=containerRef.current,scrollType=getScrollType(horizontal);state.offset=offset;state.controller.set({scroll:state.current});
...
//# sourceMappingURL=module$node_modules$$react_spring$parallax$dist$react_spring_parallax_cjs_dev.js.map
